...
class IOUMetric:
    ...
    def _fast_hist(self, label_pred, label_true):
        # 找出标签中需要计算的类别,去掉了背景
        # print("unname:",[i for i in label_true if i !=0])
        # print("len_label_true:",len([i for i in label_true if i !=0]))
        # print("len_label_pred:", len([i for i in label_pred if i != 0]))
        mask = (label_true >= 0) & (label_true < self.num_classes)
        # # np.bincount计算了从0到n**2-1这n**2个数中每个数出现的次数，返回值形状(n, n)
        # print("label_pred:",label_pred,"\n","label_true:",label_true)
        # print("mask:",mask)
        # print("bincount:",np.bincount(label_true))
        hist = np.bincount(
    ...
    def evaluate(self, predictions, gts):
        ...
        gts = gts.cpu().numpy()#numpy是cpu-only的
        # print(predictions.shape,gts.shape)#(2, 512, 512) (2, 512, 512)
        for lp, lt in zip(predictions, gts):
            # print("label_pre:{}\nlabel_true:{}".format(lp,lt))
            assert len(lp.flatten()) == len(lt.flatten())
            self.hist += self._fast_hist(lp.flatten(), lt.flatten())
        # print("self.hist:",self.hist)
        TN, FP, FN, TP = self.hist[0][0], self.hist[0][1], self.hist[1][0], self.hist[1][1]
        # miou
...
def fast_hist(a, b, n):
    # --------------------------------------------------------------------------------#
    #   a是转化成一维数组的标签，形状(H×W,)；b是转化成一维数组的预测结果，形状(H×W,);n是分割类别+1
    # --------------------------------------------------------------------------------#
    k = (a >= 0) & (a < n)
    # --------------------------------------------------------------------------------#
    #   np.bincount计算了从0到n**2-1这n**2个数中每个数出现的次数，返回值形状(n, n)
